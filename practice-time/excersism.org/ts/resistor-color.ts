function showCodex() {
  let srce: HTMLElement | null =  document.getElementById("src");
  srce.textContent = `
  // @ts-check
  //
...
export enum resistorArray {
  ...
  type Color = resistorArray;
  
  export function decodedValue([firstResistor,secondResistor]: Array<keyof typeof resistorArray>):number {
  return Number(`${resistorArray[firstResistor]}${resistorArray[secondResistor]}`);
  };